package filetransformer_test

import (
	"github.com/ScaleneZA/CryptoTaxCalculator/cmd/taxcalculator"
	"github.com/ScaleneZA/CryptoTaxCalculator/cmd/taxcalculator/ops/filetransformer"
	"io"
	"os"
	"testing"

	"github.com/stretchr/testify/assert"
)

func TestTransform(t *testing.T) {
	testCases := []struct {
		name      string
		seedFiles []string
		typ       taxcalculator.TransformType
		expected  []taxcalculator.Transaction
	}{
		{
			name:      "Basic Example",
			typ:       taxcalculator.TransformTypeBasic,
			seedFiles: []string{"./test_data/basic.csv"},
			expected: []taxcalculator.Transaction{
				{
					Transformer:       taxcalculator.TransformTypeBasic,
					Currency:          "ETH",
					DetectedType:      taxcalculator.TypeBuy,
					Amount:            0.56,
					Timestamp:         1519812503,
					WholePriceAtPoint: 100,
				},
				{
					Transformer:       taxcalculator.TransformTypeBasic,
					Currency:          "ETH",
					DetectedType:      taxcalculator.TypeBuy,
					Amount:            1.2,
					Timestamp:         1535450903,
					WholePriceAtPoint: 200,
				},
				{
					Transformer:       taxcalculator.TransformTypeBasic,
					Currency:          "ETH",
					DetectedType:      taxcalculator.TypeSell,
					Amount:            0.25,
					Timestamp:         1656410903,
					WholePriceAtPoint: 300,
				},
				{
					Transformer:       taxcalculator.TransformTypeBasic,
					Currency:          "ETH",
					DetectedType:      taxcalculator.TypeSell,
					Amount:            1.25,
					Timestamp:         1687946903,
					WholePriceAtPoint: 400,
				},
			},
		},
		{
			name:      "Basic Example - Unordered, no header happy path",
			typ:       taxcalculator.TransformTypeBasic,
			seedFiles: []string{"./test_data/basic_unordered_no_header.csv"},
			expected: []taxcalculator.Transaction{
				{
					Transformer:       taxcalculator.TransformTypeBasic,
					Currency:          "ETH",
					DetectedType:      taxcalculator.TypeBuy,
					Amount:            0.56,
					Timestamp:         1519812503,
					WholePriceAtPoint: 100,
				},
				{
					Transformer:       taxcalculator.TransformTypeBasic,
					Currency:          "ETH",
					DetectedType:      taxcalculator.TypeBuy,
					Amount:            1.2,
					Timestamp:         1535450903,
					WholePriceAtPoint: 200,
				},
				{
					Transformer:       taxcalculator.TransformTypeBasic,
					Currency:          "ETH",
					DetectedType:      taxcalculator.TypeSell,
					Amount:            0.25,
					Timestamp:         1656410903,
					WholePriceAtPoint: 300,
				},
				{
					Transformer:       taxcalculator.TransformTypeBasic,
					Currency:          "ETH",
					DetectedType:      taxcalculator.TypeSell,
					Amount:            1.25,
					Timestamp:         1687946903,
					WholePriceAtPoint: 400,
				},
			},
		},
		{
			name: "Basic Example - Two files",
			typ:  taxcalculator.TransformTypeBasic,
			seedFiles: []string{
				"./test_data/basic.csv",
				"./test_data/basic_unordered_no_header.csv",
			},
			expected: []taxcalculator.Transaction{
				{
					Transformer:       taxcalculator.TransformTypeBasic,
					Currency:          "ETH",
					DetectedType:      taxcalculator.TypeBuy,
					Amount:            0.56,
					Timestamp:         1519812503,
					WholePriceAtPoint: 100,
				},
				{
					Transformer:       taxcalculator.TransformTypeBasic,
					Currency:          "ETH",
					DetectedType:      taxcalculator.TypeBuy,
					Amount:            0.56,
					Timestamp:         1519812503,
					WholePriceAtPoint: 100,
				},
				{
					Transformer:       taxcalculator.TransformTypeBasic,
					Currency:          "ETH",
					DetectedType:      taxcalculator.TypeBuy,
					Amount:            1.2,
					Timestamp:         1535450903,
					WholePriceAtPoint: 200,
				},
				{
					Transformer:       taxcalculator.TransformTypeBasic,
					Currency:          "ETH",
					DetectedType:      taxcalculator.TypeBuy,
					Amount:            1.2,
					Timestamp:         1535450903,
					WholePriceAtPoint: 200,
				},
				{
					Transformer:       taxcalculator.TransformTypeBasic,
					Currency:          "ETH",
					DetectedType:      taxcalculator.TypeSell,
					Amount:            0.25,
					Timestamp:         1656410903,
					WholePriceAtPoint: 300,
				},
				{
					Transformer:       taxcalculator.TransformTypeBasic,
					Currency:          "ETH",
					DetectedType:      taxcalculator.TypeSell,
					Amount:            0.25,
					Timestamp:         1656410903,
					WholePriceAtPoint: 300,
				},
				{
					Transformer:       taxcalculator.TransformTypeBasic,
					Currency:          "ETH",
					DetectedType:      taxcalculator.TypeSell,
					Amount:            1.25,
					Timestamp:         1687946903,
					WholePriceAtPoint: 400,
				},
				{
					Transformer:       taxcalculator.TransformTypeBasic,
					Currency:          "ETH",
					DetectedType:      taxcalculator.TypeSell,
					Amount:            1.25,
					Timestamp:         1687946903,
					WholePriceAtPoint: 400,
				},
			},
		},
		{
			name:      "Basic Example - Multi-currency",
			typ:       taxcalculator.TransformTypeBasic,
			seedFiles: []string{"./test_data/basic_multi_currency.csv"},
			expected: []taxcalculator.Transaction{
				{
					Transformer:       taxcalculator.TransformTypeBasic,
					Currency:          "ETH",
					DetectedType:      taxcalculator.TypeBuy,
					Amount:            0.56,
					Timestamp:         1519812503,
					WholePriceAtPoint: 100,
				},
				{
					Transformer:       taxcalculator.TransformTypeBasic,
					Currency:          "ETH",
					DetectedType:      taxcalculator.TypeBuy,
					Amount:            1.2,
					Timestamp:         1535450903,
					WholePriceAtPoint: 200,
				},
				{
					Transformer:       taxcalculator.TransformTypeBasic,
					Currency:          "BTC",
					DetectedType:      taxcalculator.TypeBuy,
					Amount:            1,
					Timestamp:         1535450913,
					WholePriceAtPoint: 1000,
				},
				{
					Transformer:       taxcalculator.TransformTypeBasic,
					Currency:          "ETH",
					DetectedType:      taxcalculator.TypeSell,
					Amount:            0.25,
					Timestamp:         1656410903,
					WholePriceAtPoint: 300,
				},
				{
					Transformer:       taxcalculator.TransformTypeBasic,
					Currency:          "ETH",
					DetectedType:      taxcalculator.TypeSell,
					Amount:            1.25,
					Timestamp:         1687946903,
					WholePriceAtPoint: 400,
				},
				{
					Transformer:       taxcalculator.TransformTypeBasic,
					Currency:          "BTC",
					DetectedType:      taxcalculator.TypeSell,
					Amount:            0.5,
					Timestamp:         1687946913,
					WholePriceAtPoint: 2000,
				},
			},
		},
		{
			name:      "Luno",
			typ:       taxcalculator.TransformTypeLuno,
			seedFiles: []string{"./test_data/LUNO_XBT.csv"},
			expected: []taxcalculator.Transaction{
				{
					Transformer:       taxcalculator.TransformTypeLuno,
					Currency:          "BTC",
					DetectedType:      taxcalculator.TypeBuy,
					Amount:            0.99579,
					Timestamp:         1453347106,
					WholePriceAtPoint: 7531.708492754497,
				},
				{
					Transformer:       taxcalculator.TransformTypeLuno,
					Currency:          "BTC",
					DetectedType:      taxcalculator.TypeBuy,
					Amount:            1.37329,
					Timestamp:         1453624978,
					WholePriceAtPoint: 7281.783163060971,
				},
				{
					Transformer:       taxcalculator.TransformTypeLuno,
					Currency:          "BTC",
					DetectedType:      taxcalculator.TypeBuy,
					Amount:            0.073002,
					Timestamp:         1466570728,
					WholePriceAtPoint: 10699.980822443222,
				},
				{
					Transformer:       taxcalculator.TransformTypeLuno,
					Currency:          "BTC",
					DetectedType:      taxcalculator.TypeBuy,
					Amount:            0.38,
					Timestamp:         1466571536,
					WholePriceAtPoint: 10705,
				},
				{
					Transformer:       taxcalculator.TransformTypeLuno,
					Currency:          "BTC",
					DetectedType:      taxcalculator.TypeFee,
					Amount:            0.0038,
					Timestamp:         1466571536,
					WholePriceAtPoint: 10734.21052631579,
				},
				{
					Transformer:       taxcalculator.TransformTypeLuno,
					Currency:          "BTC",
					DetectedType:      taxcalculator.TypeBuy,
					Amount:            1.74346,
					Timestamp:         1480915521,
					WholePriceAtPoint: 11471.441845525564,
				},
				{
					Transformer:       taxcalculator.TransformTypeLuno,
					Currency:          "BTC",
					DetectedType:      taxcalculator.TypeSendInternal,
					Amount:            1,
					Timestamp:         1484542490,
					WholePriceAtPoint: 12051,
				},
				{
					Transformer:       taxcalculator.TransformTypeLuno,
					Currency:          "BTC",
					DetectedType:      taxcalculator.TypeBuy,
					Amount:            0.244261,
					Timestamp:         1484714102,
					WholePriceAtPoint: 12898.989195982984,
				},
				{
					Transformer:       taxcalculator.TransformTypeLuno,
					Currency:          "BTC",
					DetectedType:      taxcalculator.TypeFee,
					Amount:            0.002442,
					Timestamp:         1484714102,
					WholePriceAtPoint: 12895.167895167893,
				},
				{
					Transformer:       taxcalculator.TransformTypeLuno,
					Currency:          "BTC",
					DetectedType:      taxcalculator.TypeSendInternal,
					Amount:            0.0285,
					Timestamp:         1484840347,
					WholePriceAtPoint: 12710.877192982456,
				},
				{
					Transformer:       taxcalculator.TransformTypeLuno,
					Currency:          "BTC",
					DetectedType:      taxcalculator.TypeSendInternal,
					Amount:            0.0283,
					Timestamp:         1485066626,
					WholePriceAtPoint: 12991.872791519436,
				},
				{
					Transformer:       taxcalculator.TransformTypeLuno,
					Currency:          "BTC",
					DetectedType:      taxcalculator.TypeSendInternal,
					Amount:            1,
					Timestamp:         1488432689,
					WholePriceAtPoint: 16616,
				},
				{
					Transformer:       taxcalculator.TransformTypeLuno,
					Currency:          "BTC",
					DetectedType:      taxcalculator.TypeSell,
					Amount:            0.08,
					Timestamp:         1610107160,
					WholePriceAtPoint: 635178.375,
				},
				{
					Transformer:       taxcalculator.TransformTypeLuno,
					Currency:          "BTC",
					DetectedType:      taxcalculator.TypeReceiveInternal,
					Amount:            0.09034,
					Timestamp:         1619086743,
					WholePriceAtPoint: 800433.9163161389,
				},
				{
					Transformer:       taxcalculator.TransformTypeLuno,
					Currency:          "BTC",
					DetectedType:      taxcalculator.TypeReceiveInternal,
					Amount:            0.09093,
					Timestamp:         1619701433,
					WholePriceAtPoint: 803903.9920818213,
				},
				{
					Transformer:       taxcalculator.TransformTypeLuno,
					Currency:          "BTC",
					DetectedType:      taxcalculator.TypeSell,
					Amount:            0.76043057,
					Timestamp:         1619796553,
					WholePriceAtPoint: 825713.6742937623,
				},
				{
					Transformer:       taxcalculator.TransformTypeLuno,
					Currency:          "BTC",
					DetectedType:      taxcalculator.TypeSell,
					Amount:            0.52295337,
					Timestamp:         1621189915,
					WholePriceAtPoint: 669275.732939631,
				},
				{
					Transformer:       taxcalculator.TransformTypeLuno,
					Currency:          "BTC",
					DetectedType:      taxcalculator.TypeSendInternal,
					Amount:            0.00072384,
					Timestamp:         1629962522,
					WholePriceAtPoint: 718901.9672855879,
				},
				{
					Transformer:       taxcalculator.TransformTypeLuno,
					Currency:          "BTC",
					DetectedType:      taxcalculator.TypeSendInternal,
					Amount:            0.00101206,
					Timestamp:         1652554670,
					WholePriceAtPoint: 488775.36904926586,
				},
				{
					Transformer:       taxcalculator.TransformTypeLuno,
					Currency:          "BTC",
					DetectedType:      taxcalculator.TypeFee,
					Amount:            0.00002327,
					Timestamp:         1652554670,
					WholePriceAtPoint: 488611.94671250536,
				},
			},
		},
	}

	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			var files []io.Reader
			for _, sf := range tc.seedFiles {
				file, err := os.Open(sf)
				assert.NoError(t, err)
				defer file.Close()

				files = append(files, file)
			}

			ts, err := filetransformer.Transform(files, tc.typ)
			assert.NoError(t, err)

			for i, exp := range tc.expected {
				exp.UID = ts[i].UID
				assert.Equal(t, exp, ts[i])
			}
		})
	}

}

func TestTransformAll(t *testing.T) {
	testCases := []struct {
		name      string
		seedFiles map[taxcalculator.TransformType][]string
		typ       taxcalculator.TransformType
		expected  []taxcalculator.Transaction
	}{
		{
			name: "Basic Example - Two different provider files",
			typ:  taxcalculator.TransformTypeBasic,
			seedFiles: map[taxcalculator.TransformType][]string{
				taxcalculator.TransformTypeBasic: {
					"./test_data/basic.csv",
				},
				taxcalculator.TransformTypeLuno: {
					"./test_data/LUNO_XBT.csv",
				},
			},
			expected: []taxcalculator.Transaction{
				{
					Transformer:       taxcalculator.TransformTypeLuno,
					Currency:          "BTC",
					DetectedType:      taxcalculator.TypeBuy,
					Amount:            0.99579,
					Timestamp:         1453347106,
					WholePriceAtPoint: 7531.708492754497,
				},
				{
					Transformer:       taxcalculator.TransformTypeLuno,
					Currency:          "BTC",
					DetectedType:      taxcalculator.TypeBuy,
					Amount:            1.37329,
					Timestamp:         1453624978,
					WholePriceAtPoint: 7281.783163060971,
				},
				{
					Transformer:       taxcalculator.TransformTypeLuno,
					Currency:          "BTC",
					DetectedType:      taxcalculator.TypeBuy,
					Amount:            0.073002,
					Timestamp:         1466570728,
					WholePriceAtPoint: 10699.980822443222,
				},
				{
					Transformer:       taxcalculator.TransformTypeLuno,
					Currency:          "BTC",
					DetectedType:      taxcalculator.TypeBuy,
					Amount:            0.38,
					Timestamp:         1466571536,
					WholePriceAtPoint: 10705,
				},
				{
					Transformer:       taxcalculator.TransformTypeLuno,
					Currency:          "BTC",
					DetectedType:      taxcalculator.TypeFee,
					Amount:            0.0038,
					Timestamp:         1466571536,
					WholePriceAtPoint: 10734.21052631579,
				},
				{
					Transformer:       taxcalculator.TransformTypeLuno,
					Currency:          "BTC",
					DetectedType:      taxcalculator.TypeBuy,
					Amount:            1.74346,
					Timestamp:         1480915521,
					WholePriceAtPoint: 11471.441845525564,
				},
				{
					Transformer:       taxcalculator.TransformTypeLuno,
					Currency:          "BTC",
					DetectedType:      taxcalculator.TypeSendInternal,
					Amount:            1,
					Timestamp:         1484542490,
					WholePriceAtPoint: 12051,
				},
				{
					Transformer:       taxcalculator.TransformTypeLuno,
					Currency:          "BTC",
					DetectedType:      taxcalculator.TypeBuy,
					Amount:            0.244261,
					Timestamp:         1484714102,
					WholePriceAtPoint: 12898.989195982984,
				},
				{
					Transformer:       taxcalculator.TransformTypeLuno,
					Currency:          "BTC",
					DetectedType:      taxcalculator.TypeFee,
					Amount:            0.002442,
					Timestamp:         1484714102,
					WholePriceAtPoint: 12895.167895167893,
				},
				{
					Transformer:       taxcalculator.TransformTypeLuno,
					Currency:          "BTC",
					DetectedType:      taxcalculator.TypeSendInternal,
					Amount:            0.0285,
					Timestamp:         1484840347,
					WholePriceAtPoint: 12710.877192982456,
				},
				{
					Transformer:       taxcalculator.TransformTypeLuno,
					Currency:          "BTC",
					DetectedType:      taxcalculator.TypeSendInternal,
					Amount:            0.0283,
					Timestamp:         1485066626,
					WholePriceAtPoint: 12991.872791519436,
				},
				{
					Transformer:       taxcalculator.TransformTypeLuno,
					Currency:          "BTC",
					DetectedType:      taxcalculator.TypeSendInternal,
					Amount:            1,
					Timestamp:         1488432689,
					WholePriceAtPoint: 16616,
				},
				{
					Transformer:       taxcalculator.TransformTypeBasic,
					Currency:          "ETH",
					DetectedType:      taxcalculator.TypeBuy,
					Amount:            0.56,
					Timestamp:         1519812503,
					WholePriceAtPoint: 100,
				},
				{
					Transformer:       taxcalculator.TransformTypeBasic,
					Currency:          "ETH",
					DetectedType:      taxcalculator.TypeBuy,
					Amount:            1.2,
					Timestamp:         1535450903,
					WholePriceAtPoint: 200,
				},
				{
					Transformer:       taxcalculator.TransformTypeLuno,
					Currency:          "BTC",
					DetectedType:      taxcalculator.TypeSell,
					Amount:            0.08,
					Timestamp:         1610107160,
					WholePriceAtPoint: 635178.375,
				},
				{
					Transformer:       taxcalculator.TransformTypeLuno,
					Currency:          "BTC",
					DetectedType:      taxcalculator.TypeReceiveInternal,
					Amount:            0.09034,
					Timestamp:         1619086743,
					WholePriceAtPoint: 800433.9163161389,
				},
				{
					Transformer:       taxcalculator.TransformTypeLuno,
					Currency:          "BTC",
					DetectedType:      taxcalculator.TypeReceiveInternal,
					Amount:            0.09093,
					Timestamp:         1619701433,
					WholePriceAtPoint: 803903.9920818213,
				},
				{
					Transformer:       taxcalculator.TransformTypeLuno,
					Currency:          "BTC",
					DetectedType:      taxcalculator.TypeSell,
					Amount:            0.76043057,
					Timestamp:         1619796553,
					WholePriceAtPoint: 825713.6742937623,
				},
				{
					Transformer:       taxcalculator.TransformTypeLuno,
					Currency:          "BTC",
					DetectedType:      taxcalculator.TypeSell,
					Amount:            0.52295337,
					Timestamp:         1621189915,
					WholePriceAtPoint: 669275.732939631,
				},
				{
					Transformer:       taxcalculator.TransformTypeLuno,
					Currency:          "BTC",
					DetectedType:      taxcalculator.TypeSendInternal,
					Amount:            0.00072384,
					Timestamp:         1629962522,
					WholePriceAtPoint: 718901.9672855879,
				},
				{
					Transformer:       taxcalculator.TransformTypeLuno,
					Currency:          "BTC",
					DetectedType:      taxcalculator.TypeSendInternal,
					Amount:            0.00101206,
					Timestamp:         1652554670,
					WholePriceAtPoint: 488775.36904926586,
				},
				{
					Transformer:       taxcalculator.TransformTypeLuno,
					Currency:          "BTC",
					DetectedType:      taxcalculator.TypeFee,
					Amount:            0.00002327,
					Timestamp:         1652554670,
					WholePriceAtPoint: 488611.94671250536,
				},
				{
					Transformer:       taxcalculator.TransformTypeBasic,
					Currency:          "ETH",
					DetectedType:      taxcalculator.TypeSell,
					Amount:            0.25,
					Timestamp:         1656410903,
					WholePriceAtPoint: 300,
				},
				{
					Transformer:       taxcalculator.TransformTypeBasic,
					Currency:          "ETH",
					DetectedType:      taxcalculator.TypeSell,
					Amount:            1.25,
					Timestamp:         1687946903,
					WholePriceAtPoint: 400,
				},
			},
		},
	}

	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			files := make(map[taxcalculator.TransformType][]io.Reader)
			for typ, fs := range tc.seedFiles {
				for _, sf := range fs {
					file, err := os.Open(sf)
					assert.NoError(t, err)
					defer file.Close()

					files[typ] = append(files[typ], file)
				}
			}

			ts, err := filetransformer.TransformAll(files)
			assert.NoError(t, err)

			for i, exp := range tc.expected {
				exp.UID = ts[i].UID
				assert.Equal(t, exp, ts[i])
			}
		})
	}

}
